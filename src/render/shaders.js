...
//
// Brightness is derived from the CAMERA distance, not baked from the Sun
// distance: apparent magnitude m = M + 5*log10(d_pc) - 5. Flying towards a star
// therefore makes it brighter and bigger, which is what makes a fly-through
// read as motion.
...
fn vs_main(
	...
	let brightness: f32 = flux / (1.0 + flux);   // Reinhard roll-off, never clips

	// Size follows the magnitude difference, not the tone-mapped flux: after
	// tone mapping every star brighter than the exposure would be exactly as
	// large as every other one. The clamp keeps a 4 px floor/ceiling band of
	// sizes instead of letting Sirius fill the screen.
	let sizePx: f32 = camera.params.y * clamp(1.0 - 0.35 * magDiff, 0.35, 3.0);
	let quadPx: f32 = clamp(sizePx, 1.5, camera.params.z);
	let fade: f32 = min(sizePx / 1.5, 1.0);

	let corner: vec2f = cornerOffset(vid);
	let offset: vec2f = corner * (quadPx * 0.5) * camera.viewport.zw;
	let onePxAlpha: f32 = select(1.0, fade, sizePx < 1.5);
	...
	out.uv = corner;
	out.color = color;
	out.brightness = brightness * onePxAlpha;
	return out;
}

@fragment
fn fs_main(in: VertexOut) -> @location(0) vec4f {
	// Soft radial profile; the sprite is a disc, not a square.
	let r2: f32 = dot(in.uv, in.uv);
	if (r2 > 1.0) { discard; }
	let falloff: f32 = (1.0 - r2) * (1.0 - r2);
	let alpha: f32 = in.brightness * falloff;
	return vec4f(in.color * alpha, alpha);   // premultiplied
}
`;
...
// is Milestone 3 (plan.md §14). `star-sprite` is the only one the renderer uses.
const WIRED_SHADERS = ['star-sprite'];
