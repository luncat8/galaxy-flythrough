...
---
Task ID: camera-quaternion-unlimited
Agent: Arena.ai Agent Mode
Task: Improve the fly camera so it is not angle-limited; review the implementation and fix issues found.

Work Log:
- Plan first: updated plan.md §0.1.1 from the old single yaw/pitch model to quaternion orientation. `yaw`/`pitch` are now diagnostics only; `PITCH_LIMIT` is an `Infinity` compatibility export.
- src/core/camera.js: replaced yaw/pitch integration and pitch clamps with a unit quaternion. Mouse look rotates around the current camera axes, orbit entry minimally rotates the quaternion to aim at the target, fly movement uses the quaternion forward/right/up basis, and `buildViewProj()` uses quaternion right/up directly instead of rebuilding them from `forward × worldUp`.
- Kept the existing fly/orbit mode model: orbit position is still `target − orbitDistance · forward`, velocity is zeroed on orbit entry, and wheel/speed/dolly behaviour is unchanged.
- src/main.js: preallocates `statsText.orientation` so the new `getState(out).orientation` field does not allocate during overlay updates. src/index.html help now says fly E/Q are local up/down.
- experiments/camera-test.js: updated the camera oracle to use the quaternion basis instead of yaw/pitch, added pole-crossing projection checks, added no-clamp loop checks, and changed continuity assertions to compare quaternion orientation.

Stage Summary:
- `python3 scripts/run.py camera`: PASS — 108/108 checks.
- `python3 scripts/run.py all-tests`: PASS.
- Review found and fixed two regressions before sign-off: the old view matrix rebuilt right/up from world-up (would still fail at the pole), and the overlay state object needed a preallocated quaternion array to preserve the no-allocation contract.
- Next: browser-check pointer-lock feel around vertical/upside-down orientations.
